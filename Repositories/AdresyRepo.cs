    ...
    public static async Task<Index<AdresyKVolbam>> CreateAutocompleteForAdresyAsync()
    {
        using (DbEntities db = new DbEntities())
        {
            ...
            var adresy = await db.AdresyKVolbam.FromSqlRaw(
                @"select ovm.Nazev NazevUradu, amo.NazevObce ObecUradu, amo.Psc PscUradu, 
                         amo.NazevUlice UliceUradu, amo.CisloDomovni CisloDomovniUradu, ovm.IdDS DatovkaUradu,
                         am.NazevObce Obec, am.Psc, am.NazevUlice Ulice, am.CisloDomovni, am.KodAdm id
                   ...
                   where typovmid in (5,6,7,8)
                     and stavsubjektu = 1
                     and ovm.Zkratka not in ('LIBAVA','BOLETICE','KPRAHA','Plzen','UstinL','Ostrava','KARLVARY',
