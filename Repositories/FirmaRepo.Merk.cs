...
namespace HlidacStatu.Repositories
{
    public static partial class FirmaRepo
    {
        ...
        {
            static string token = Config.GetWebConfigValue("MerkAPIKey");
            ...
            {
                return string.Format("https://www.detail.cz/firma/cz-{0}", Util.ParseTools.IcoToMerkIco(ico));
            }
            ...
            public static Firma FromName(string name)
            {
                ...
                string n1 = TextUtil.RemoveDiacritics(Firma.JmenoBezKoncovky(name)).ToLowerInvariant();
                var r1 = res
                    .Where(r => TextUtil.RemoveDiacritics(Firma.JmenoBezKoncovky(r.name)).ToLowerInvariant() == n1)
                    .FirstOrDefault();
            ...
            private static CoreCompanyStructure[] Search(string url, string query)
            {
                try
                {
                    ...
                    CoreCompanyStructure[] res =
                        Newtonsoft.Json.JsonConvert.DeserializeObject<CoreCompanyStructure[]>(req);
                    if (res == null)
            ...
            private static string Request(string url)
            {
                using (Devmasters.Net.HttpClient.URLContent http = new Devmasters.Net.HttpClient.URLContent(url))
                {
                    ...
                    try
                    {
                        var a = http.GetContent(Encoding.UTF8).Text;
                        return a;
                    }
            ...
            public class CoreCompanyStructure
            {
                public Firma ToFirma()
                {
                    ...
                    f.DIC = vatno;
                    f.DatovaSchranka = databox_ids?.ToArray() ?? new string[] { };
