    ...
    public class RecalculateItemRepo
    {
        ...
        )
        {
            await Devmasters.Batch.Manager.DoActionForAllAsync<int>(Enumerable.Range(0, int.MaxValue - 1), async xx =>
                ...
                monitor: new MonitoredTaskRepo.ForBatchAsync("Downloader ", "RecalculateTasks from queue ")
            );
            ...
            Firma f, RecalculateItem.StatisticsTypeEnum statsType, string provokeBy, int deep)
        {
            ...
            var parents = (await f.ParentFirmyAsync(DS.Graphs.Relation.CharakterVazbyEnum.VlastnictviKontrola, DS.Graphs.Relation.AktualnostType.Nedavny))
                .UnionBy(await f.ParentFirmyAsync(DS.Graphs.Relation.CharakterVazbyEnum.Uredni, DS.Graphs.Relation.AktualnostType.Nedavny), m => m.ICO);
