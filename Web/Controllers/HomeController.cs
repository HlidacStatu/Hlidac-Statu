...
namespace HlidacStatu.Web.Controllers
{
    public partial class HomeController : Controller
    {
        ...
        public ActionResult Analyza(string ids)
        {
            ...
            string[] ids_arr = ids.Split(new string[] { ",", ";", " ", Environment.NewLine },
...
";
            ...
            [FromQuery] string f)
        {
            ...
            if (string.IsNullOrEmpty(id))
            {
                ...
                var nameId = Devmasters.RegexUtil.GetRegexGroupValue(f,
                    @"\d{2} \\ (?<nameid>\w* - \w* (-\d{1,3})?) - small\.jpg", "nameid");
            ...
            try
            {
                ...
                Visit.AddVisit(path,
                    Visit.IsCrawler(Request.Headers[HeaderNames.UserAgent])
                        ? Visit.VisitChannel.Crawler
                        : Visit.VisitChannel.Web);
            }
            catch (Exception e)
            {
        ...
        public async Task<ActionResult> sendFeedbackMail(string typ, string email, string txt, string url, bool? auth,
            string data)
        {
...
";
        ...
        public async Task<ActionResult> KopiePrilohy(string Id, string hash, string secret, bool forcePDF = false)
        {
            ...
            var hashlist =
                await SmlouvaRepo.GetPartValuesAsync<string>(Id, "prilohy.hash.value", "$.Prilohy..hash.Value");
            ...
            if (platnyZaznam == false)
            {
                if (User.IsInRole("Admin") == false &&
                    secret != Devmasters.Config.GetWebConfigValue("LocalPrilohaUniversalSecret"))
                {
                    if (string.IsNullOrEmpty(secret)) //pokus jak se dostat k znepristupnene priloze
            ...
            {
                return File(await System.IO.File.ReadAllBytesAsync(fn), "application/pdf",
                    string.IsNullOrWhiteSpace(priloha.nazevSouboru)
                        ? $"{model.Id}_smlouva.pdf"
                        : priloha.nazevSouboru + ".pdf");
            }
            else
        ...
        public ActionResult SnadneHledani()
        {
            ...
            {
                query += " " + platce.Where(m => !string.IsNullOrWhiteSpace(m.Value)).Select(m => m.Key + ":" + m.Value)
                    .First();
            }
            ...
            {
                query += " " + prijemce.Where(m => !string.IsNullOrWhiteSpace(m.Value))
                    .Select(m => m.Key + ":" + m.Value).First();
            }
            ...
            string? obsahujeSlova, string? presnyTermin, string? typ)
        {
            bool hasAny =
                ...
                !string.IsNullOrWhiteSpace(obsahujeSlova) ||
                !string.IsNullOrWhiteSpace(presnyTermin) ||
            ...
            if (hasAny)
            {
                ...
                if (!string.IsNullOrWhiteSpace(icoPrijemce))
                    parts.Add($"ico_prijemce:{icoPrijemce}");
                if (datumOd.HasValue)
                    parts.Add($"datum_od:{datumOd:yyyy-MM-dd}");
                if (datumDo.HasValue)
                    parts.Add($"datum_do:{datumDo:yyyy-MM-dd}");
                if (castkaOd.HasValue)
                    parts.Add($"castka_od:{castkaOd}");
                if (castkaDo.HasValue)
                    parts.Add($"castka_do:{castkaDo}");
                if (!string.IsNullOrWhiteSpace(osoba))
                    parts.Add($"osobaid:{osoba}");
                if (!string.IsNullOrWhiteSpace(presnyTermin))
        ...
        public async Task<ActionResult> Hledat2(string q, string order)
        {
            var res = await XLib.Search
                .GeneralSearchAsync(q, 1, PartsToSearch.All, false, order, this.User,
                    smlouvySize: Repositories.Searching.SearchDataResult<object>.DefaultPageSizeGlobal);
            AuditRepo.Add(
                Audit.Operations.UserSearch
                , User?.Identity?.Name
                , HlidacStatu.Util.RealIpAddress.GetIp(HttpContext)?.ToString()
        ...
        public async Task<ActionResult> Hledat(string q, string order)
        {
            ...
            var res = await XLib.Search
                .GeneralSearchAsync(q, 1, PartsToSearch.All, showBeta, order, this.User,
                    smlouvySize: Repositories.Searching.SearchDataResult<object>.DefaultPageSizeGlobal);
            AuditRepo.Add(
                Audit.Operations.UserSearch
                , User?.Identity?.Name
            ...
            string img, string rat = "16x9", string res = "1200x628", string col = "")
        {
            ...
            {
                Firma fi = await FirmaCache.GetAsync(v);
                if (fi?.Valid == true)
                {
