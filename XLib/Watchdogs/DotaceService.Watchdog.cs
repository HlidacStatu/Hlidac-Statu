...
namespace HlidacStatu.XLib.Watchdogs
{
    ...
    {
        public class WatchdogDotace : IWatchdogProcessor
        {
            ...
            protected override EmailMessageFromTemplate CreateNotificationEmail(string txtTemplate, string htmlTemplate, WatchDogProcessor.Result res, string toEmail = null, int maxCount = int.MaxValue)
            {
            ...
            {
                query += " AND datumAktualizace:" + string.Format("[{0} TO {1}]", Repositories.Searching.Tools.ToElasticDate(fromDate), Repositories.Searching.Tools.ToElasticDate(toDate)); //AND platnyZaznam:1
            ...
            {
                var query = "datumAktualizace:" + string.Format("[* TO {0}]", Repositories.Searching.Tools.ToElasticDate(toDate));
